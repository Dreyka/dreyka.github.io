...
// Функция обработчик для управления ракетками
    function tennisRacquetMove(EO) {
        EO = EO || window.event;
        if (EO.keyCode === 16 && tennisRacquet1Pos.y > 0) {
            tennisRacquet1Pos.y -= tennisRacquet1Pos.speedY;
            tennisRacquet1Pos.update();
        }
        if (EO.keyCode === 17 && tennisRacquet1Pos.y < (tennisPar.field.height[0] - tennisPar.tennisRacquet1.height[0])) {
            tennisRacquet1Pos.y += tennisRacquet1Pos.speedY;
            tennisRacquet1Pos.update();
        }
        if (EO.keyCode === 38 && tennisRacquet2Pos.y > 0) {
            tennisRacquet2Pos.y -= tennisRacquet2Pos.speedY;
            tennisRacquet2Pos.update();
        }
        if (EO.keyCode === 40 && tennisRacquet2Pos.y < (tennisPar.field.height[0] - tennisPar.tennisRacquet2.height[0])) {
            tennisRacquet2Pos.y += tennisRacquet2Pos.speedY;
            tennisRacquet2Pos.update();
        }
    }
